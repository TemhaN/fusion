      ...
      <div class="row mt-5">
        <div class="col-md-4">
          <a href="{{ route('films.edit', $film->id) }}">
            <img src="{{ $film->link_img }}" alt="Film"
              style="min-height:500px; max-height: 500px; border-radius:25px ">
          </a>
        </div>
        <div class="card-body" style="margin-left: -150px">
          <div class="font-weight-bold film-name d-flex flex-col" style="font-size: 3rem">
            ...
            <a class="ml-4" style="margin-top: 15px">
              <h1
                style="background: linear-gradient(to right, #FFD700, #d08801); -webkit-background-clip: text; color: transparent;">
                {{ $averageRating }}
                <i class="fas fa-regular fa-star ml-3"
                  style="background: linear-gradient(to right, #FFD700, #efa620); -webkit-background-clip: text; color: transparent;"></i>
              </h1>
              </з>
          </div>
          ...
          <div style="line-height: 20px">
            ...
            <div class="d-flex flex-row justify-content-end btn-secondary2" style="gap: 30px">
              <a class="btn btn-secondary button-save" href="{{ $film->link_kinopoisk }}">Ссылка на Кинопоиск
                </ф>
                <a class="btn btn-secondary button-save" href="{{ $film->link_video }}">Ссылка на
                  трейлер</a>
            </div>
          </div>
        </div>
                ...
                <div class="d-flex" style="gap: 30px">
                  <form action="{{ route('reviews.toggle', $review->id) }}" method="POST">
                    ...
                    @if ($review->is_approved)
                      <button type="submit" class="btn btn-warning button-save">Снять
                        одобрение</button>
                    @else
      ...
      search.addEventListener('keyup', function() {
        ...
        for (let i = 0; i < reviews.length; i++) {
          ...
          let match = queries.every(function(q) {
            return nickname.toLowerCase().includes(q) || messages.some(message => message
              .toLowerCase().includes(q)) || isApproved.includes(q);
          });
