...
<div class="row">
  <div class="col-12">
    <div class="mb-3">
      <button type="button" class="btn btn-success button-savet" data-toggle="modal" data-target="#createCategoryModal">
        Добавить
      </button>
    </div>
    <div class="card">
        ...
        <table class="table table-hover text-nowrap table-borderless table-striped">
          ...
          <tbody class="categoriesfilms-list">
            ...
            @if($categoryfilm->category)
              <tr>
                <th>
                    <a href="#" data-toggle="modal" data-target="#editCategoryModal{{ $categoryfilm->id }}">{{ $categoryfilm->film->name }}</a>
                    <div class="modal fade" id="editCategoryModal{{ $categoryfilm->id }}" tabindex="-1" role="dialog" aria-labelledby="editCategoryModalLabel{{ $categoryfilm->id }}" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h5 class="modal-title" id="editCategoryModalLabel{{ $categoryfilm->id }}">Редактировать категорию</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                              <span aria-hidden="true" style="font-size: 2.5rem; color: #333333;">×</span>
                            </button>
                          </div>
                          <form action="{{ route('categoryfilm.update', $categoryfilm->id) }}" method="POST">
                            ...
                            <div class="modal-body">
                              <div class="form-group">
                                ...
                                <select class="form-control select2" style="width: 100%;" name="film_id">
                                  @foreach ($films as $film)
                                    <option value="{{ $film->id }}" {{ $film->id == $categoryfilm->film_id ? 'selected' : '' }}>
                                      {{ $film->name }}
                                    </option>
                                  @endforeach
                                </select>
                              ...
                              <div class="form-group">
                                ...
                                <select class="form-control select2" style="width: 100%;" name="category_id">
                                  @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" {{ $category->id == $categoryfilm->category_id ? 'selected' : '' }}>
                                      {{ $category->name }}
                                    </option>
                                  @endforeach
                                </select>
