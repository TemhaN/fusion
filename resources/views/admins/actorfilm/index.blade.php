                  ...
                  <tr>
                    <th>
                      <a href="#" data-toggle="modal"
                        data-target="#editActorModal{{ $actorfilm->id }}">{{ $actorfilm->film->name }}</a>
                      <div class="modal fade" id="editActorModal{{ $actorfilm->id }}" tabindex="-1" role="dialog"
                        aria-labelledby="editActorModalLabel{{ $actorfilm->id }}" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title" id="editCategoryModalLabel{{ $actorfilm->id }}">Редактировать Актёра
                              </h5>
                              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true" style="font-size: 2.5rem; color: #333333;">×</span>
                              </button>
                              ...
                              <div class="modal-body">
                                <div class="form-group">
                                  ...
                                  <select class="form-control select2" style="width: 100%;" name="film_id">
                                    @foreach ($films as $film)
                                      <option value="{{ $film->id }}"
                                        {{ $film->id == $actorfilm->film_id ? 'selected' : '' }}>
                                        {{ $film->name }}
                                      </option>
                                    @endforeach
                                  </select>
                                ...
                                <div class="form-group">
                                  ...
                                  <select class="form-control select2" style="width: 100%;" name="actor_id">
                                    @foreach ($actors as $actor)
                                      <option value="{{ $actor->id }}"
                                        {{ $actor->id == $actorfilm->actor_id ? 'selected' : '' }}>
                                        {{ $actor->name }}
                                      </option>
                                    @endforeach
                                  </select>
